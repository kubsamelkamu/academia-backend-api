import { Injectable } from '@nestjs/common';
import {
  CoordinatorEvaluatorNotificationDeliveryMethod,
  EvaluationStage,
  EvaluatorProjectEvaluationStatus,
  NotificationSeverity,
  ProjectStatus,
  UserStatus,
} from '@prisma/client';

import { PrismaService } from '../../prisma/prisma.service';

type EligibleEvaluatorItem = {
  evaluatorUserId: string;
  fullName: string;
  email: string;
  avatarUrl: string | null;
  assignedProjectsCount: number;
  pendingEvaluationsCount: number;
  submittedEvaluationsCount: number;
};

@Injectable()
export class CoordinatorEvaluatorNotificationsRepository {
  constructor(private readonly prisma: PrismaService) {}

  private get prismaClient(): any {
    return this.prisma as any;
  }

  async findUserDepartmentContext(userId: string) {
    return this.prisma.user.findUnique({
      where: { id: userId },
      select: {
        id: true,
        tenantId: true,
        departmentId: true,
      },
    });
  }

  async departmentExistsInTenant(departmentId: string, tenantId: string) {
    const department = await this.prisma.department.findFirst({
      where: {
        id: departmentId,
        tenantId,
      },
      select: { id: true },
    });

    return Boolean(department);
  }

  async findEligibleDepartmentEvaluators(params: {
    tenantId: string;
    departmentId: string;
    stage: EvaluationStage;
    evaluatorUserIds?: string[];
    search?: string;
  }): Promise<EligibleEvaluatorItem[]> {
    const assignments = await this.prisma.projectEvaluator.findMany({
      where: {
        ...(params.evaluatorUserIds?.length
          ? { evaluatorUserId: { in: params.evaluatorUserIds } }
          : {}),
        project: {
          tenantId: params.tenantId,
          departmentId: params.departmentId,
          status: ProjectStatus.ACTIVE,
        },
        evaluator: {
          tenantId: params.tenantId,
          departmentId: params.departmentId,
          deletedAt: null,
          status: UserStatus.ACTIVE,
          ...(params.search
            ? {
                OR: [
                  { firstName: { contains: params.search, mode: 'insensitive' } },
                  { lastName: { contains: params.search, mode: 'insensitive' } },
                  { email: { contains: params.search, mode: 'insensitive' } },
                ],
              }
            : {}),
        },
      },
      select: {
        projectId: true,
        evaluatorUserId: true,
        evaluator: {
          select: {
            id: true,
            firstName: true,
            lastName: true,
            email: true,
            avatarUrl: true,
          },
        },
        project: {
          select: {
            evaluatorEvaluations: {
              where: { stage: params.stage },
              select: {
                evaluatorUserId: true,
                status: true,
              },
            },
          },
        },
      },
      orderBy: [
        { evaluator: { firstName: 'asc' } },
        { evaluator: { lastName: 'asc' } },
        { createdAt: 'asc' },
      ],
    });

    const byEvaluator = new Map<string, EligibleEvaluatorItem & { projectIds: Set<string> }>();

    for (const assignment of assignments) {
      const existing = byEvaluator.get(assignment.evaluatorUserId) ?? {
        evaluatorUserId: assignment.evaluator.id,
        fullName: `${assignment.evaluator.firstName} ${assignment.evaluator.lastName}`.trim(),
        email: assignment.evaluator.email,
        avatarUrl: assignment.evaluator.avatarUrl,
        assignedProjectsCount: 0,
        pendingEvaluationsCount: 0,
        submittedEvaluationsCount: 0,
        projectIds: new Set<string>(),
      };

      if (!existing.projectIds.has(assignment.projectId)) {
        existing.projectIds.add(assignment.projectId);
        existing.assignedProjectsCount += 1;

        const evaluation = assignment.project.evaluatorEvaluations.find(
          (item) => item.evaluatorUserId === assignment.evaluatorUserId
        );

        if (evaluation?.status === EvaluatorProjectEvaluationStatus.SUBMITTED) {
          existing.submittedEvaluationsCount += 1;
        } else {
          existing.pendingEvaluationsCount += 1;
        }
      }

      byEvaluator.set(assignment.evaluatorUserId, existing);
    }

    return Array.from(byEvaluator.values())
      .map(({ projectIds: _projectIds, ...item }) => item)
      .sort((left, right) => left.fullName.localeCompare(right.fullName));
  }

  async createCampaignWithRecipients(params: {
    tenantId: string;
    departmentId: string;
    createdByUserId: string;
    stage: EvaluationStage;
    recipientMode: any;
    deliveryMethod: CoordinatorEvaluatorNotificationDeliveryMethod;
    priority: NotificationSeverity;
    subject: string;
    message: string;
    recipients: Array<{
      evaluatorUserId: string;
      email: string;
      fullName: string;
    }>;
  }) {
    return this.prisma.$transaction(async (tx) => {
      const campaign = await (tx as any).coordinatorEvaluatorNotificationCampaign.create({
        data: {
          tenantId: params.tenantId,
          departmentId: params.departmentId,
          createdByUserId: params.createdByUserId,
          stage: params.stage,
          recipientMode: params.recipientMode,
          deliveryMethod: params.deliveryMethod,
          priority: params.priority,
          subject: params.subject,
          message: params.message,
          requestedRecipientsCount: params.recipients.length,
          totalReachedCount: params.recipients.length,
        },
      });

      if (params.recipients.length > 0) {
        await (tx as any).coordinatorEvaluatorNotificationRecipient.createMany({
          data: params.recipients.map((recipient) => ({
            campaignId: campaign.id,
            tenantId: params.tenantId,
            departmentId: params.departmentId,
            evaluatorUserId: recipient.evaluatorUserId,
            email: recipient.email,
            fullName: recipient.fullName,
          })),
        });
      }

      return campaign;
    });
  }

  async updateRecipientDelivery(params: {
    campaignId: string;
    evaluatorUserId: string;
    inAppStatus?: any;
    emailStatus?: any;
    inAppNotificationId?: string | null;
    emailProviderMessageId?: string | null;
    emailFailureReason?: string | null;
  }) {
    return this.prismaClient.coordinatorEvaluatorNotificationRecipient.update({
      where: {
        campaignId_evaluatorUserId: {
          campaignId: params.campaignId,
          evaluatorUserId: params.evaluatorUserId,
        },
      },
      data: {
        ...(params.inAppStatus ? { inAppStatus: params.inAppStatus } : {}),
        ...(params.emailStatus ? { emailStatus: params.emailStatus } : {}),
        ...(params.inAppNotificationId !== undefined
          ? { inAppNotificationId: params.inAppNotificationId }
          : {}),
        ...(params.emailProviderMessageId !== undefined
          ? { emailProviderMessageId: params.emailProviderMessageId }
          : {}),
        ...(params.emailFailureReason !== undefined
          ? { emailFailureReason: params.emailFailureReason }
          : {}),
      },
    });
  }

  async updateCampaignCounts(params: {
    campaignId: string;
    inAppDeliveredCount: number;
    inAppFailedCount: number;
    emailQueuedCount: number;
    emailAcceptedCount: number;
    emailDeliveredCount: number;
    emailFailedCount: number;
  }) {
    return this.prismaClient.coordinatorEvaluatorNotificationCampaign.update({
      where: { id: params.campaignId },
      data: {
        inAppDeliveredCount: params.inAppDeliveredCount,
        inAppFailedCount: params.inAppFailedCount,
        emailQueuedCount: params.emailQueuedCount,
        emailAcceptedCount: params.emailAcceptedCount,
        emailDeliveredCount: params.emailDeliveredCount,
        emailFailedCount: params.emailFailedCount,
      },
    });
  }

  async listCampaignsPaged(params: {
    tenantId: string;
    departmentId: string;
    skip: number;
    take: number;
    stage?: EvaluationStage;
    deliveryMethod?: CoordinatorEvaluatorNotificationDeliveryMethod;
    priority?: NotificationSeverity;
    search?: string;
  }) {
    const where = {
      tenantId: params.tenantId,
      departmentId: params.departmentId,
      ...(params.stage ? { stage: params.stage } : {}),
      ...(params.deliveryMethod ? { deliveryMethod: params.deliveryMethod } : {}),
      ...(params.priority ? { priority: params.priority } : {}),
      ...(params.search
        ? {
            OR: [
              { subject: { contains: params.search, mode: 'insensitive' as const } },
              { message: { contains: params.search, mode: 'insensitive' as const } },
            ],
          }
        : {}),
    };

    const [items, total] = await this.prisma.$transaction([
      this.prismaClient.coordinatorEvaluatorNotificationCampaign.findMany({
        where,
        orderBy: [{ createdAt: 'desc' }],
        skip: params.skip,
        take: params.take,
        include: {
          createdBy: {
            select: {
              id: true,
              firstName: true,
              lastName: true,
              avatarUrl: true,
            },
          },
        },
      }),
      this.prismaClient.coordinatorEvaluatorNotificationCampaign.count({ where }),
    ]);

    return { items, total };
  }

  async findCampaignById(params: { campaignId: string; tenantId: string; departmentId: string }) {
    return this.prismaClient.coordinatorEvaluatorNotificationCampaign.findFirst({
      where: {
        id: params.campaignId,
        tenantId: params.tenantId,
        departmentId: params.departmentId,
      },
      include: {
        createdBy: {
          select: {
            id: true,
            firstName: true,
            lastName: true,
            avatarUrl: true,
          },
        },
        recipients: {
          orderBy: [{ fullName: 'asc' }],
          include: {
            inAppNotification: {
              select: {
                readAt: true,
              },
            },
          },
        },
      },
    });
  }

  async summarizeCampaigns(params: {
    tenantId: string;
    departmentId: string;
    stage?: EvaluationStage;
  }) {
    const where = {
      tenantId: params.tenantId,
      departmentId: params.departmentId,
      ...(params.stage ? { stage: params.stage } : {}),
    };

    const [totals, campaignCount, deliveredCampaignCount] = await Promise.all([
      this.prismaClient.coordinatorEvaluatorNotificationCampaign.aggregate({
        where,
        _sum: {
          totalReachedCount: true,
          inAppDeliveredCount: true,
          emailQueuedCount: true,
          emailAcceptedCount: true,
          emailDeliveredCount: true,
          emailFailedCount: true,
        },
      }),
      this.prismaClient.coordinatorEvaluatorNotificationCampaign.count({ where }),
      this.prismaClient.coordinatorEvaluatorNotificationCampaign.count({
        where: {
          ...where,
          OR: [
            { inAppDeliveredCount: { gt: 0 } },
            { emailAcceptedCount: { gt: 0 } },
            { emailDeliveredCount: { gt: 0 } },
          ],
        },
      }),
    ]);

    return {
      totalSent: campaignCount,
      delivered: deliveredCampaignCount,
      totalReached: totals._sum.totalReachedCount ?? 0,
      inAppDelivered: totals._sum.inAppDeliveredCount ?? 0,
      emailQueued: totals._sum.emailQueuedCount ?? 0,
      emailAccepted: totals._sum.emailAcceptedCount ?? 0,
      emailDelivered: totals._sum.emailDeliveredCount ?? 0,
      emailFailed: totals._sum.emailFailedCount ?? 0,
    };
  }
}
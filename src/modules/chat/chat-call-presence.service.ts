...
export class ChatCallPresenceService implements OnModuleDestroy {
  ...
  async startCall(params: { roomId: string; projectGroupId: string; userId: string }) {
    const client = this.getClient();
  ...
  async joinCall(params: { roomId: string; userId: string }) {
    ...
    if (active !== '1') {
      return { roomId: params.roomId, active: false, participantCount: 0 };
    }
    ...
    const participantCount = await client.scard(participantsKey);
    return { roomId: params.roomId, active: true, participantCount };
  }
  ...
  async leaveCall(params: { roomId: string; userId: string }) {
    ...
    if (participantCount > 0) {
      ...
      return { roomId: params.roomId, ended: false, participantCount };
    }

    await client.multi().del(metadataKey).del(participantsKey).exec();
    return { roomId: params.roomId, ended: true, participantCount: 0 };
  }
  ...
  async endCall(params: { roomId: string; endedByUserId: string }) {
    ...
    const pipeline = client.multi().del(this.metadataKey(params.roomId)).del(participantsKey);
  ...
  async leaveAllCallsForUser(userId: string) {
    ...
    const results: Array<{ roomId: string; ended: boolean; participantCount: number }> = [];
